...
class AutonomousResearchService:
    ...
    def __init__(self):
        ...
        if getattr(settings, "OPENAI_API_KEY", None):
            try:
                kwargs = {"api_key": settings.OPENAI_API_KEY, "timeout": 15.0}
    ...
    def _invoke_llm(self, prompt: str, system_prompt: str, max_tokens: int = 1500) -> Optional[str]:
        """Invokes available neural LLM (Gemini, NVIDIA NIM, or OpenAI) for intelligence generation."""
        # Try Gemini first
        ...
        if self.nvidia_client and getattr(settings, "NVIDIA_API_KEY", None):
            try:
                ...
                if resp.choices:
                    ...
                    text = (getattr(msg, "content", None) or getattr(msg, "reasoning_content", "") or "").strip()
                    if text:
                        return text
            except Exception:
                pass
        ...
        # Try OpenAI
        if self.openai_client and getattr(settings, "OPENAI_API_KEY", None):
            try:
    ...
    async def run_autonomous_research(
        ...
        """
        Executes bounded autonomous investigation run with neural LLM synthesis.
        """
        ...
        # Step 6: Neural LLM Report Synthesis
        entity_summaries = [
            {"id": n["id"], "name": n.get("properties", {}).get("full_name", n["id"]), "label": n.get("label"), "role": n.get("properties", {}).get("role")}
            for n in relevant_entities
        ]
        edge_summaries = [
            {"from": e.get("from_id"), "to": e.get("to_id"), "rel": e.get("rel_type"), "confidence": e.get("confidence")}
            for e in discovered_connections[:15]
        ]

        llm_prompt = f"""Case Reference: {case_id}
Investigation Objective: {objective}
Traversal Depth: {max_depth} (Bounded)
...
{json.dumps(edge_summaries, indent=2)}

Flagged Contradictions & Anomalies:
{json.dumps(contradictions, indent=2)}

Audited Cryptographic Evidence Sources:
{json.dumps(list(source_evidence_ids)[:8], indent=2)}

Please generate an investigative intelligence dossier with:
1. Executive Summary
2. Target Nexus Analysis (focusing on {primary_entity_name})
3. Critical Operational Connections
4. Contradiction & Telemetry Evaluation
5. Primary Hypothesis Formulation with Confidence Score
6. Actionable Leads for Lead Investigators"""

        system_prompt = (
            "You are Byomkesh, the Autonomous Forensic Intelligence Engine of Constellation. "
            "You synthesize grounded investigative reports strictly from verified graph telemetry. "
            "Citing evidence IDs and entity identifiers is mandatory. Do not hallucinate."
        )

        neural_report = self._invoke_llm(llm_prompt, system_prompt, max_tokens=1400)

        hyp_statement = f"Entity '{primary_entity_name}' functioned as the coordinating intermediary for the syndicate network."
        hyp_confidence = 0.86

        if neural_report:
            report_text = neural_report
            # Extract hypothesis from report if present
            hyp_match = re.search(r"Hypothesis(?:\s+\d+)?:?\s*[\"']?([^\n\r\"']{20,160})[\"']?", neural_report, re.IGNORECASE)
            if hyp_match:
...
#### 2. Key Hypotheses
- [{hyp_id}] {hyp_statement} (Confidence: {int(hyp_confidence*100)}%)
...
"""
    ...
    async def challenge_hypothesis(
        ...
        # Neural LLM Adversarial Review
        revised_confidence = None
        new_status = None
        verdict = None

        challenge_llm_prompt = f"""Hypothesis Under Scrutiny: "{hyp['statement']}"
Current Confidence Rating: {old_confidence}
Current Supporting Evidence IDs: {list(supporting_ids)}

Investigator Challenge Rationale:
"{challenge_req.challenge_statement}"

Contested / Counter-Evidence IDs Submitted:
{list(contested_ids)}

Evaluate the impact of this challenge. Return ONLY valid JSON in this exact structure:
{{
  "revised_confidence": <number between 0.10 and 0.95>,
  "status": "<refuted|under_review|challenged|supported>",
  "verdict": "<concise 1-sentence forensic summary>",
  "detailed_reasoning": "<thorough analytical explanation>"
}}"""

        system_review_prompt = (
            "You are an adversarial investigative review magistrate for the Constellation Intelligence Platform. "
            "You evaluate investigator challenges to determine whether counter-evidence or contradictions refute, weaken, or sustain a hypothesis. "
            "Respond strictly in valid JSON."
        )

        neural_eval = self._invoke_llm(challenge_llm_prompt, system_review_prompt, max_tokens=600)
        if neural_eval:
            try:
                clean_json = re.sub(r"^```(?:json)?\s*|\s*```$", "", neural_eval.strip(), flags=re.MULTILINE).strip()
                data = json.loads(clean_json)
                if "revised_confidence" in data and isinstance(data["revised_confidence"], (int, float)):
                    revised_confidence = round(float(data["revised_confidence"]), 2)
                    new_status = str(data.get("status", "challenged")).lower()
                    if new_status not in ("refuted", "under_review", "challenged", "supported"):
        ...
        if revised_confidence is None:
            # Deterministic graph-topological fallback
            overlap_count = len(supporting_ids.intersection(contested_ids))
            contradiction_weight = 0.18 if overlap_count > 0 else 0.08
            evidence_weight = len(contested_ids) * 0.10
            penalty = min(0.65, evidence_weight + (overlap_count * 0.20) + contradiction_weight)
            ...
            verdict = f"Evaluated counter-evidence against hypothesis graph: revised confidence from {int(old_confidence*100)}% to {int(revised_confidence*100)}%."
        ...
        challenge_entry = {
            ...
            "verdict": verdict or f"Re-evaluated hypothesis: confidence updated {int(old_confidence*100)}% -> {int(revised_confidence*100)}% based on counter-evidence."
        }
