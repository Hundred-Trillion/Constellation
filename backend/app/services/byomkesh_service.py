...
class ByomkeshAgent:
    ...
    async def _node_construct_explanation(self, state: ByomkeshState) -> Dict[str, Any]:
        ...
        for row in results:
            from_node = row.get("p") or row.get("n")
            edge = row.get("r")
            to_node = row.get("other") or row.get("m")
            if from_node and edge and to_node:
        ...
        if not citations:
            ...
            # Try OpenAI if others not available
            if not answer_text and self.openai_client and getattr(settings, "OPENAI_API_KEY", None):
                try:
                    ...
                    logger.warning(f"OpenAI synthesis bypassed: {e}")
