...
class IngestionService:
    ...
    async def ingest_file_as_evidence(
        ...
        safe_filename = re.sub(r'[^A-Za-z0-9_.\-]', '_', raw_basename)
        if not safe_filename:
            safe_filename = "unnamed_artifact.dat"

        evidence_store_abs = os.path.abspath(settings.EVIDENCE_STORE_DIR)
        dest_filename = f"{evidence_id}_{safe_filename}"
        dest_path = os.path.abspath(os.path.join(evidence_store_abs, dest_filename))
        
        if not dest_path.startswith(evidence_store_abs):
            raise ValueError(f"Security restriction: Potential path traversal in filename '{filename}'.")
