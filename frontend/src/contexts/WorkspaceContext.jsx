...
export function WorkspaceProvider({ children }) {
  ...
  const addRopeConnection = (sourceOrConfig, maybeTargetId, maybeLabel = 'COORDINATES_WITH', options = {}) => {
    let sourceId, targetId, label, confidence, notes;

    if (sourceOrConfig && typeof sourceOrConfig === 'object') {
      sourceId = sourceOrConfig.sourceId || sourceOrConfig.source || sourceOrConfig.from_id;
      targetId = sourceOrConfig.targetId || sourceOrConfig.target || sourceOrConfig.to_id;
      label = sourceOrConfig.relType || sourceOrConfig.label || 'COORDINATES_WITH';
      confidence = sourceOrConfig.confidence !== undefined ? sourceOrConfig.confidence : 0.94;
      notes = sourceOrConfig.notes || '';
    } else {
      sourceId = sourceOrConfig;
      targetId = maybeTargetId;
      label = maybeLabel || 'COORDINATES_WITH';
      confidence = options.confidence !== undefined ? options.confidence : 0.94;
      notes = options.notes || '';
    }
    ...
    const localEdgeId = `rope-${Date.now()}`;
    const newEdge = {
      id: localEdgeId,
      source: sourceId,
      target: targetId,
    ...
    api.createRelationship({
      ...
      const serverRelId = res?.id || res?.data?.id;
      if (serverRelId) {
        setCanvasEdges(currentEdges => currentEdges.map(e => 
          e.id === localEdgeId ? { ...e, backendRelId: serverRelId } : e
        ));
      }
  ...
  const removeEdge = (edgeId) => {
    const targetEdge = canvasEdges.find(e => e.id === edgeId);
    const nextEdges = canvasEdges.filter(e => e.id !== edgeId);
    setCanvasEdges(nextEdges);

    // Persist severance to backend graph and audit ledger
    const idToDelete = targetEdge?.backendRelId || edgeId;
    api.deleteRelationship(idToDelete).catch(err => {
