...
export function AuthProvider({ children }) {
  ...
  const demoLogin = useCallback(async (role = 'admin') => {
    ...
    const defaultCredentials = {
      admin: { username: 'admin', password: 'password', full_name: 'Chief Intelligence Director', role: 'admin' },
      investigator: { username: 'investigator', password: 'password', full_name: 'Lead Intelligence Officer', role: 'investigator' },
      analyst: { username: 'analyst', password: 'password', full_name: 'Senior Intelligence Analyst', role: 'read_only' }
    };
    const target = defaultCredentials[role] || defaultCredentials.admin;
    try {
      const data = await api.login(target.username, target.password);
      if (data?.user) {
        setUser(data.user);
        localStorage.setItem('constellation_user', JSON.stringify(data.user));
        return data.user;
      ...
      console.warn('Real demo login error:', err);
    }
    const fallbackUser = { id: `usr_${target.username}`, username: target.username, full_name: target.full_name, role: target.role };
