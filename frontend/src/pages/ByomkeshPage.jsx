    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        {
          ...
          role: 'byomkesh',
          reasoning: [
            'Connection attempt to Byomkesh backend service failed.',
            'The backend API server may not be running or is unreachable from this browser.',
            'Please ensure the FastAPI backend is started on localhost:8000.'
          ],
          answer: `Unable to reach the Byomkesh reasoning engine.\n\nThe backend API server is not responding. To use the AI co-pilot:\n\n1. Start the backend server: \`cd backend && uvicorn app.main:app --reload\`\n2. Ensure an LLM API key is configured in your .env file (GEMINI_API_KEY, NVIDIA_API_KEY, or OPENAI_API_KEY).\n3. Retry your query once the backend is online.`,
          citations: [],
          confidence: 'N/A',
          provenance: 'CONNECTION_ERROR',
          timestamp: 'Just now'
        }
      ]);
      setExpandedReasoning(prev => ({ ...prev, [aiMsgId]: true }));
