...
@router.get("/users")
async def list_users(current_user: UserResponse = Depends(require_role(["admin"]))):
    """
    List all registered users from the database. Requires Administrator privilege.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
...
@router.post("/users")
async def create_user(user_in: UserCreate, current_user: UserResponse = Depends(require_role(["admin"]))):
    """
    Admin endpoint to add a new user to the SQLite database. Requires Administrator privilege.
    """
